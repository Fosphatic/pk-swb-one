...
<article class="uk-article">
    ...
    <?php if ($image = $post->get('image.src')): ?>
        <a class="tm-article-featured-image uk-cover-background uk-box-shadow-large" href="<?= $view->url('@blog/id', ['id' => $post->id]) ?>" style="background-image: url(<?= $view->url($image) ?>);" uk-scrollspy="cls: uk-animation-slide-bottom-medium;  delay: 250; repeat: false"></a>
    <?php endif ?>

    <div class="tm-article-content uk-position-relative">

        <div class="tm-article-date uk-text-center uk-box-shadow-small">
        ...
        <ul class="uk-subnav uk-margin-bottom-remove">
            ...
            <li><a class="uk-button-primary uk-button-large" href="<?= $view->url('@blog/id', ['id' => $post->id]) ?>"><?= __('Read more') ?></a></li>
            <?php endif ?>

            <?php if ($post->isCommentable() || $post->comment_count) : ?>
            <li><a class="uk-button-primary uk-button-large" href="<?= $view->url('@blog/id#comments', ['id' => $post->id]) ?>"><?= _c('{0} No comments|{1} %num% Comment|]1,Inf[ %num% Comments', $post->comment_count, ['%num%' => $post->comment_count]) ?></a></li>
            <?php endif ?>
